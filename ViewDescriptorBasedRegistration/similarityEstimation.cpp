...
//{
//	Mat dst1, dst2;
//	//cvtColor(src1, src1, CV_BGR2GRAY);     //×ª»»Îª»Ò¶ÈÍ¼Ïñ
//	src1.convertTo(dst1, CV_32FC1);       //×ª»»Îª32Î»¸¡µãÐÍ
...
bool SimilarityEstimation::PreProcessByOpenCV(Mat& src, Mat& dst/*, int x, int y, int w, int h*/)
{
	...
	for (int i = 0; i < dst.rows; i++){
		for (int j = 0; j < dst.cols; j++){
			...
			double val[9] = { 0 }, mid = 0;//·Ö±ðÎª´æ·Å´°¿ÚÖÐËùÓÐ»Ò¶ÈÖµµÄÊý×éºÍÖÐÖµ
			int num = 0;//¼ÇÂ¼Êµ¼Ê´°¿ÚÖÐÓÐ¼¸¸öÏñËØµã
			//±éÀúµ±Ç°ÏñËØµãÎªÖÐÐÄµÄ3X3´°¿Ú
			for (int k = -1; k <= 1; k++)
				for (int m = -1; m <= 1; m++)
					if (i + k >= 0 && i + k < dst.rows && j + m >= 0 && j + m < dst.cols){//ÔÚÍ¼ÏñÉÏ£¬Ã»ÓÐ³¬³ö±ß½ç
...
//¼ÆËãÃ¿¸öÆ«ÒÆÏÂµÄÏàËÆÐÔ£¬¶¼Îª¿Õ(-1.0)»ò¶¼²»Îª¿Õ(>0.0)ÔòÏàËÆÐÔ¼ÓÒ»
bool SimilarityEstimation::similarityByOccupation(ViewDescriptor &src1, ViewDescriptor &src2, PhaseSimilarityResult &sr)
{
	...
	{
		cout << "¼ÆËãÏàËÆÐÔµÄÁ½ÌØÕ÷Î¬¶È²»Ò»ÖÂ£¡" << endl;
		return false;
	}
	if (src1.viewDepth.empty() || src2.viewDepth.empty())
	{
		cout << "¼ÆËãÏàËÆÐÔµÄÁ½ÌØÕ÷Éî¶ÈÖµÎ´³õÊ¼»¯£¡" << endl;
		return false;
	}
...
//¼ÆËãÃ¿¸öÆ«ÒÆÏÂµÄÏàËÆÐÔ£¬ÒÔ×î¸ßÕÚµ²½ÇÅÐ¶ÏÏàËÆ¸öÊý£¬ÊäÈëÓ¦¸ÃÊÇÎ´ÇÐ¸î¹ýµÄÍ¼Ïñ£¨45*90£©£¬ÊäÈëµÄTLSÌØÕ÷Ó¦Ô¤ÏÈÈ¥Ôë
...
bool SimilarityEstimation::similarityBySkyLineAndDepth(ViewDescriptor &src1, ViewDescriptor &src2, PhaseSimilarityResult &sr)
{
	...
	for (int deltaCol = 0; deltaCol < src1.Nh; ++deltaCol)
	{
		...
		for (int j = 0; j < src1.Nh; ++j)
		{
			...
			//Èç¹ûÆäÖÐÒ»ÁÐ¸ß¶È½ÇÎª¿Õ£¬ÔòÏàËÆ¶È¼ÇÎª0
			...
			//ÅÐ¶Ï×î¸ßÕÚµ²½ÇÖ®²îÊÇ·ñÔÚ¿¼ÂÇ·¶Î§ÄÚ
...
bool SimilarityEstimation::similarityDP(Skyline3DContour &sky1, Skyline3DContour &sky2, PhaseSimilarityResult &sr)
{
	if (sky1.Nh != sky2.Nh)
	{
		cout << "Ìì¼ÊÏßÂÖÀª·Ö±æÂÊ²»Ò»ÖÂ" << endl;
		return false;
	}
...
PhaseSimilarityResult SimilarityEstimation::similaritySkylineOneDP(Skyline3DContour &sky1, Skyline3DContour &sky2, int deltaCol, int width)
{
	...
	for (int k = 1; k < skySize; ++k)
	{
		...
		for (int i = k - width + 1; i < k; ++i)//Öð¸ö¸üÐÂµ½¶Ô½ÇÏßÔªËØ¼õÒ»
		{
			if (i<0) continue;
		...
		for (int j = k - width + 1; j <= k; ++j)//Öð¸ö¸üÐÂµ½¶Ô½ÇÏßÔªËØ
		{
			if (j<0) continue;
